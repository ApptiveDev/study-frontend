...
- 본문에서 React 컴포넌트는 **순수 함수**로 설계되어야 한다고 말씀드렸습니다. 왜 React는 렌더링 함수가 순수해야 한다고 강조할까요?
- JavaScript의 **클로저(Closure)**는 무엇인가요? `useEffect` 내부에서 사용하는 변수가 "오래된" 값을 가리키게 되는 stale closure 현상은 왜 발생하며, 어떻게 피할 수 있을까요?
- React는 `useEffect`의 **의존성 배열**에 포함된 값들이 변경되었는지 어떻게 판단할까요? (`Object.is`?) 객체나 배열, 함수를 의존성 배열에 직접 포함시키면 왜 의도한 대로 동작하지 않나요?
- `useMemo`와 `useCallback`은 어떤 Hook이고, 어떤 상황에서 사용하면 좋을까요?
- **Hook은 컴포넌트 함수의 최상위에서만 호출해야 하며, 반복문이나 조건문 안에서 호출하면 안 됩니다. 왜 이런 제약이 있을까요? React는 내부적으로 어떤 방식으로 Hook의 호출 순서를 추적할까요? (중요)**

## 과제: 채팅 입력 임시 저장 기능 만들기

자세한 설명은 [해당 프로젝트](../../projects/2-Hook)를 참고해주세요.
